...
PR: #77
Merge SHA: 5f0bd04
Evidence: CI run 32374291130 — unit, typecheck/lint, production build, pgTAP,
Chromium product journeys, Supabase-backed learning journey (all pass)
Date: 2026-08-20
```
...
- [x] VLR-001 real blueprint attempt resolution
- [x] VLR-002 real blueprint support resolution
- [x] VLR-003 generic delayed-review plan resolution
...
- [x] session lesson version and resolved blueprint cannot disagree
      Structural rather than a runtime comparison: `resolveOwnedLessonBlueprint`
      derives the blueprint *from* the session's own lesson version, so there is one
      source instead of two to check against each other. The arbitrary journey proves
      it end to end.
- [x] update stale docs claiming modules are fully connected
      `AGENTS.md` corrected in the same change; the import-graph guard
      (`tests/integration/module-reachability.test.ts`) now keeps it honest.
