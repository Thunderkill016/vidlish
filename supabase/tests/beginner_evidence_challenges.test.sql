-- Beginner evidence must be created only from a server-issued challenge.
...
select is(
  has_function_privilege(
    'authenticated',
    'public.record_beginner_challenge_evidence(uuid,uuid,boolean)',
    'EXECUTE'
  ),
  false,
...
select isnt(
  ...
  null::timestamptz,
  'a successful independent challenge can bank independence'
);
...
select isnt(
  ...
  null::timestamptz,
  'a later supported/failed challenge does not erase prior independence'
);
...
select is(
  (select successful_retrievals
   ...
   where owner_user_id = 'd1111111-1111-4111-8111-111111111111'
     and item_key = 'water'),
  1,
  'only the independent challenge increments successful retrievals'
);
