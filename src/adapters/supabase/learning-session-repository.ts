...
const reviewItemRowSchema = z
  .object({
    ...
    transfer_attempted_at: z.string().nullable(),
...
function mapReviewItem(
  ...
  return learningReviewItemStateSchema.parse({
    ...
    transferAttemptedAt: row.transfer_attempted_at ?? null,
  ...
  implements LearningSessionRepository, LearningReviewRepository
{
  ...
  async listScheduled(ownerUserId: string) {
    const result = await this.client
      ...
      .select(
        "owner_user_id,item_key,source_lesson_version_id,exposure_count,attempt_count,successful_retrievals,last_outcome,last_seen_at,next_review_at,last_delayed_transfer_at,last_independent_at,transfer_attempted_at,transfer_succeeded_at,review_state",
      )
      .eq("owner_user_id", ownerUserId)
