...
export default async function DashboardPage() {
  ...
  const continueRow =
    lessonRows.find((row) => row.percent > 0 && row.percent < 100) ?? lessonRows[0];
  const completedCount = progressSummaries.filter((summary) => summary.completedAt).length;
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.5fr)_minmax(280px,0.8fr)]">
        {continueRow ? (
          <Card className="overflow-hidden p-0">
            <div className="border-b border-[var(--border)] bg-[var(--primary-wash)] px-6 py-5">
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--primary)]">
                {continueRow.percent > 0 ? "Tiếp tục bài học" : "Bài học gần nhất"}
              </p>
              <h2 className="mt-2 text-2xl font-bold">{continueRow.lesson.titleVi}</h2>
          ...
          <Card className="flex min-h-72 flex-col justify-between bg-[var(--primary-wash)]">
            <div>
              <p className="text-sm font-semibold text-[var(--primary)]">Bắt đầu từ một đoạn bạn thật sự muốn hiểu</p>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-1">
          <Card className="space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-[var(--accent)]">Ôn tập dài hạn</p>
                <h2 className="mt-1 text-xl font-bold">Chưa bật queue production</h2>
              </div>
            ...
            <p className="text-sm text-[var(--muted-foreground)]">
              Learning Model v2 đã có session/attempt bền vững; delayed review vẫn là gate tiếp theo nên Vidlish chưa giả vờ có “mastery”.
            </p>
            <Link href="/review" className="inline-flex min-h-10 items-center text-sm font-semibold text-[var(--primary)]">
              Xem cách Review sẽ hoạt động →
            </Link>
          </Card>

          {activeJobs.length > 0 ? (
            <Card className="space-y-3 border-dashed">
              <p className="text-sm font-semibold text-[var(--accent)]">Đang tạo</p>
              <h2 className="text-lg font-bold">{activeJobs[0].videoTitle}</h2>
              <p className="text-sm text-[var(--muted-foreground)]">
                Job vẫn chạy khi bạn rời trang. Mở lại để xem tiến trình thật.
              </p>
              <Link href={`/jobs/${activeJobs[0].id}`} className="inline-flex min-h-10 items-center text-sm font-semibold text-[var(--primary)]">
                Xem tiến trình →
              </Link>
            </Card>
          ) : (
            <Card className="space-y-3">
              ...
              <Link href="/create" className="inline-flex min-h-10 items-center text-sm font-semibold text-[var(--primary)]">
                Dán URL YouTube →
              </Link>
            </Card>
          )}
      ...
      <section className="space-y-4" aria-labelledby="learning-snapshot-heading">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-[var(--accent)]">Learning snapshot</p>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <Card className="space-y-1 p-5">
            ...
            <p className="text-sm text-[var(--muted-foreground)]">Bài học đã lưu</p>
          </Card>
          <Card className="space-y-1 p-5">
            <p className="text-3xl font-bold">{completedCount}</p>
            <p className="text-sm text-[var(--muted-foreground)]">Phiên Study Mode đã hoàn tất</p>
          </Card>
          <Card className="space-y-1 p-5">
            <p className="text-3xl font-bold">{activeJobs.length}</p>
            <p className="text-sm text-[var(--muted-foreground)]">Job đang xử lý</p>
          </Card>
        </div>
      </section>
    </div>
