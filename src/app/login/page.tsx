...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    if (error) setMessage(`Lỗi Google Login: ${error.message}`);
    setLoading(false);
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    else setMessage("✅ Đã gửi Magic Link!");
    setLoading(false);
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="max-w-md w-full bg-white rounded-xl shadow-sm p-8 border border-gray-100">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-gray-900">Đăng Nhập Nếp V4</h1>
          <p className="text-gray-500 mt-2">Hệ thống học tập Evidence-Centered</p>
        </div>
        ...
        <div className="space-y-4">
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium py-2.5 rounded-lg transition disabled:opacity-50"
          >
        ...
        <div className="mt-6 text-center text-sm text-gray-500">
          *Chúng tôi sử dụng Magic Link (đăng nhập không cần mật khẩu) để bảo mật tuyệt đối.
        </div>
      </div>
    </div>
  );
